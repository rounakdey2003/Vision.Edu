...
if uploaded_file is not None:
    ...
    if file_extension in ['jpg', 'jpeg', 'png']:
        ...
        dy = st.slider("dy", min_value=0, max_value=2, value=1)
        ...
        if dx == 1 and dy == 0:
            ...
            direction = "Second derivative in X"
        elif dx == 0 and dy == 2:
            direction = "Second derivative in Y"
        else:
            direction = f"dx={dx}, dy={dy}"
        
        sobel = cv.Sobel(opencv_image.copy(), ddepth=ddepth, dx=dx, dy=dy, ksize=ksize)
        if ddepth == cv.CV_8U:
        ...
        success, encoded_img = cv.imencode('.png', sobel)
        if success:
            img_bytes = BytesIO(encoded_img.tobytes())
            st.download_button(
                label="Download Image",
                data=img_bytes.getvalue(),
                file_name=f"{uploaded_file.name.split('.')[0]}_sobel.png",
                mime="image/png"
            )
...
# Parameters: dx={dx}, dy={dy}, ksize={ksize}, ddepth={ddepth} ({depth_name})
sobel = cv2.Sobel(data, 
                ddepth={ddepth}, 
                dx={dx}, 
                dy={dy}, 
                ksize={ksize})
...
if {ddepth} == cv2.CV_8U:
    sobel = cv2.uint8(sobel)
else:
    sobel = cv2.uint8(cv2.absolute(sobel))

# Display the Sobel edge detected image
fig, ax = plt.subplots()
ax.imshow(sobel, cmap='gray')
ax.set_title('Sobel Edge - {direction} | K:{ksize}x{ksize} | Depth:{depth_name}')
ax.axis('off')
plt.show()
        ''', language='python')
        ...
        video_dy = st.slider("Video dy", min_value=0, max_value=2, value=1)
        ...
        elif video_dx == 2 and video_dy == 0:
            video_direction = "Second derivative in X"
        elif video_dx == 0 and video_dy == 2:
            video_direction = "Second derivative in Y"
        else:
        ...
        if uploaded_file is not None:
            ...
            output_filename = f"processed_{uploaded_file.name.split('.')[0]}_sobel.mp4"
            ...
            while data_video.isOpened():
                ...
                if ret:
                    sobel = cv.Sobel(frame, ddepth=video_ddepth, dx=video_dx, dy=video_dy, ksize=video_ksize)
                    if video_ddepth == cv.CV_8U:
                        processed_sobel = np.uint8(sobel)
                    else:
                        processed_sobel = np.uint8(np.absolute(sobel))
                    
                    out.write(processed_sobel)
                    
                    frame_placeholder.image(processed_sobel, caption=f"Processing {frame_num + 1}/{frame_count} Frames at {fps} FPS")
            ...
            if os.path.exists(output_filename):
                with open(output_filename, "rb") as file:
                    st.download_button(
                        ...
                        data=file.read(),
                        file_name=f"{uploaded_file.name.split('.')[0]}_sobel.mp4",
                        mime="video/mp4"
                    )
...
while data_video.isOpened():
    ...
    if ret:
        ...
        # Sobel Edge Detection: {video_direction}
        # Parameters: dx={video_dx}, dy={video_dy}, ksize={video_ksize}, ddepth={video_ddepth} ({video_depth_name})
        sobel = cv2.Sobel(frame, 
                        ddepth={video_ddepth}, 
                        dx={video_dx}, 
                        dy={video_dy}, 
                        ksize={video_ksize})
